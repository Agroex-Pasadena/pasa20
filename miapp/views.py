...
from miapp.models import Enfermedad, Review, Suggestion, Mireview, Enfermedadmaes, Enfermedadmara, Reviewmaes, Reviewmara, Enfermedadsoho, Reviewsoho, Enfermedadsota, Reviewsota, Enfermedadsora, Reviewsora, Enfermedadtriho, Reviewtriho, Enfermedadtripla, Reviewtripla, Enfermedadtries, Reviewtries
...
from miapp.forms import EnfermedadmaizForm, EnfermedadmaesmaizForm, EnfermedadmaramaizForm, EnfermedadsohomaizForm, EnfermedadsotamaizForm, EnfermedadsoramaizForm, EnfermedadtrihomaizForm, EnfermedadtriplamaizForm, EnfermedadtriesmaizForm
...
# A new analisis. Assume Data is CORRECT!
@csrf_exempt
...
def preferencetriesPage(request):
    return render(request, 'analisistries.html')

def newPreferencetries(request):
    result = clipsMatchPreferencetries(request.POST)
    print(result)
    response = []
    if result != None:
        for val in result.split('---'):
            if "," in val:
                val2 = val.split(',')
                enfermedadtries = Enfermedadtries.objects.get(id=int(val2[0]))
                response.append({"id": enfermedadtries.id, "name": enfermedadtries.name, "images": enfermedadtries.images, "description": enfermedadtries.description,
                    "planta": val2[1], "sintomaAAA": val2[2], "sintomaBB": val2[3], 
                    "sintomaCC": val2[4],  
                    "stars": float(val2[5])})
...
def clipsMatchPreferencetries(data):
    # Preference
    preference = '(preference ' +\
                 '(planta "'+data['planta']+'") ' +\
                 '(sintoma-aa "'+data['sintomaAA']+'") ' +\
    ...
    # CLIPS
    clips.Clear()
    clips.BatchStar(settings.CLIPS_DIR + "/templatestries.clp")
    if os.path.isfile(settings.CLIPS_DIR + "/enfermedadestries.clp"):
        clips.BatchStar(settings.CLIPS_DIR + "/enfermedadestries.clp")
    if os.path.isfile(settings.CLIPS_DIR + "/reviewstries.clp"):
        clips.BatchStar(settings.CLIPS_DIR + "/reviewstries.clp")
    #if os.path.isfile(settings.CLIPS_DIR + "/suggestionstries.clp"):
    #    clips.BatchStar(settings.CLIPS_DIR + "/suggestionstries.clp")
    clips.BatchStar(settings.CLIPS_DIR + "/reglastries.clp")
    clips.Reset()
    clips.Assert(preference)
    clips.Run()
    return clips.StdoutStream.Read()
    ...
    # New Review, Assume Data is CORRECT
@csrf_exempt
def processReviewstries(request):
    if request.method == "POST":
        id = insertReviewtriesIntoDatabase(request.POST)
        insertReviewtriesIntoClips(request.POST, id)
        return HttpResponse('')
    else:
        response = []
        reviews = Reviewtries.objects.filter(enfermedadId=int(request.GET['enfermedadId'])).order_by("-id")
        for review in reviews:
            response.append({"id": review.id, "comment": review.comment, "reviewer": review.reviewer,
                    "createdTime": review.createdTime})
        return JsonResponse(response, safe=False)
...
def insertReviewtriesIntoDatabase(data):
    review = Reviewtries(reviewer=data['reviewer'],
                comment=data['comment'],
                stars=float(data['stars']),
                enfermedadName=data['enfermedadName'],
                enfermedadId=int(data['enfermedadId']),
...
def insertReviewtriesIntoClips(data, id):
    # check if a fact-file exists
    FactsFile = settings.CLIPS_DIR + "/reviewstries.clp"
    if not os.path.isfile(FactsFile):
        file = open(FactsFile, 'w+')
        file.write("(deffacts reviews)\n")
        file.close()
...
@login_required
def nuevotriesEnfermedadPage(request):
    return render(request, 'agregartries.html')


# Create New Dish, Assume Data is CORRECT!
@csrf_exempt
def creartriesEnfermedad(request):
    id = insertartriesNuevaEnfermedad(request.POST)
#    insertIntoClips(id, request.POST)
    return HttpResponse('')


# Utility Functions
def insertartriesNuevaEnfermedad(data):
    images = ast.literal_eval(data['images'])
    imageNum = 0;
    for image in images:
        imageNum = imageNum + (image != 'null')

    enfermedadtries = Enfermedadtries(name=data['name'],
                description=data['description'],
                images=imageNum,
                planta=data['planta'],
                sintomaAA=data['sintomaAA'],
                sintomaBB=data['sintomaBB'],
                sintomaCC=data['sintomaCC'])
    enfermedadtries.save()

    index = 0
    for image in images:
        if image != 'null':
            index = index + 1
            createtriesImage(enfermedadtries.id, index, image)

    return enfermedadtries.id


def createtriesImage(id, index, image):
    imgCore = image.split(',')[1]
    imgFile = open(settings.DISHTRIES_IMAGE_DIR + "/" + str(id) + "_" + str(index) + ".jpeg", "wb")
    imgFile.write(imgCore.decode('base64'))
    imgFile.close()

    # Create square image
    img = Image.open(settings.DISHTRIES_IMAGE_DIR + "/" + str(id) + "_" + str(index) + ".jpeg")
    longer_side = max(img.size)
    thumb = Image.new('RGBA', (longer_side, longer_side), (255, 255, 255, 0))
    thumb.paste(
        img, ((longer_side - img.size[0]) / 2, (longer_side - img.size[1]) / 2)
    )
    thumb.save(settings.DISHTRIES_IMAGE_DIR + "/" + str(id) + "_" + str(index) + "_square.jpeg")


@csrf_exempt
def escrituratriesPage(request):
    enfermedadtriesmaiz = Enfermedadtries.objects.latest('id')
    return render(request, 'escrituratries.html', {'enfermedadtriesmaiz':enfermedadtriesmaiz})       


@csrf_exempt
def escrituratries(request):
  #  insertEscrituraIntoDatabase(request.POST)
  if request.method == "POST":
    insertEscrituratriesIntoClips()
    return render(request, 'escrituratries.html')


# INICIO NUEVO BONJOUR---------

def insertEscrituratriesIntoClips():
    FactsFile = settings.CLIPS_DIR + "/enfermedadestries.clp"
    if not os.path.isfile(FactsFile):
        file = open(FactsFile, 'w+')
        file.write("(deffacts enfermedades)\n")
        file.close()
    ...
    #enfermedades = Dish.objects.all()
    #nuevalinea
    suggestions = Enfermedadtries.objects.all()
    #finnuevalinea
    lines = ['(deffacts enfermedades\n']
    for suggestion in suggestions:
        lines.append('    (enfermedad '
...
@login_required
def reportetriesmaizPage(request):
    enfermedadtriesmaiz = Enfermedadtries.objects.all()
    return render(request, 'reportetriesmaiz.html', {'enfermedadtriesmaiz':enfermedadtriesmaiz})


@login_required
def reportecalificacionmaiztriesPage(request):
    reviewmaiz = Reviewtries.objects.order_by('-stars')
    return render(request, 'reportecalificacionmaiztries.html', {'reviewmaiz':reviewmaiz})


@login_required
def accionestriesPage(request):
    enfermedadtriesmaiz = Enfermedadtries.objects.all()
    return render(request, 'accionestries.html', {'enfermedadtriesmaiz':enfermedadtriesmaiz}) 


#inicio codigofacilitoooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo

def enfermedadtriesmaiz_view(request):
    if request.method == 'POST':
        form = EnfermedadtriesmaizForm(request.POST) 
        if form.is_valid():
            form.save()
        return redirect('enfermedadtriesmaiz:index')
    else:
        form = EnfermedadtriesmaizForm()
    return render(request, 'enfermedadtriesmaiz_form.html', {'form':form})  


def enfermedadtriesmaiz_edit(request,id_enfermedadtriesmaiz):
    enfermedadtriesmaiz = Enfermedadtries.objects.get(id=id_enfermedadtriesmaiz)     
    if request.method == 'GET':
        form = EnfermedadtriesmaizForm(instance=enfermedadtriesmaiz)
    else:
        form = EnfermedadtriesmaizForm(request.POST, instance=enfermedadtriesmaiz)
        if form.is_valid():
            form.save()
        return redirect('accionestriesPage')
    return render(request, 'enfermedadtriesmaiz_form.html', {'form':form}) 


def enfermedadtriesmaiz_delete(request, id_enfermedadtriesmaiz):
    enfermedadtriesmaiz = Enfermedadtries.objects.get(id=id_enfermedadtriesmaiz)
    if request.method == 'POST':
        enfermedadtriesmaiz.delete()
        return redirect('accionestriesPage')
    return render(request,'enfermedadtriesmaiz_delete.html', {'enfermedadtriesmaiz':enfermedadtriesmaiz}) 
